...
class AuthNS {
  ...
  init() {
    ...
    for (const rr of zrrs) {
      const zsigA = dnssec.sign(zkey, zpriv, [rr]);
    ...
    // Sign SOA
    const skrrs = this.server.zone.get(this.domain, types.SOA);
    const ksigSOA = dnssec.sign(kkey, kpriv, skrrs);
    this.server.zone.fromString(ksigSOA.toString());

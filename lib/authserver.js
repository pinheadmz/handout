...
const {AuthServer, dnssec, constants, tlsa, util} = bns;
...
class AuthNS {
  ...
  init() {
    ...
    // Create A record for TLD and CNAME for all subdomains
    zone.fromString(
      `${this.domain} 21600 IN A ${this.host}`
    );
    zone.fromString(
      `*.${this.domain} 21600 IN CNAME ${this.domain}`
    );
    ...
    for (const [, rrs] of zone.wild.rrs)
      zone.insert(dnssec.sign(ZSKkey, ZSKpriv, rrs));
